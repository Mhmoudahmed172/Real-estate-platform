...
export function PaymentDetailsPage() {
  ...
  return <motion.div {...pageMotion}><PageContainer>
    <PageHeader eyebrow="المدفوعات" title={`دفعة #${payment.id}`} description="تفاصيل الدفعة وتسجيل السداد والتسويات المرتبطة بها." actions={<div className="flex flex-wrap gap-2"><Button className="rounded-full" onClick={() => setRecordOpen(true)}>تسجيل دفعة</Button><Button className="rounded-full" variant="outline" onClick={() => setAdjustOpen(true)}>إضافة تسوية</Button>{firstTransactionId ? <Button className="rounded-full" isLoading={receiptLoading} variant="outline" onClick={() => void downloadReceipt(firstTransactionId)}><Download aria-hidden="true" className="size-4" /> تحميل إيصال</Button> : null}</div>} />
