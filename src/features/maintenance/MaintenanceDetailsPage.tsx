...
export function MaintenanceDetailsPage() {
  ...
  return <motion.div {...pageMotion}><PageContainer><PageHeader eyebrow="الصيانة" title={`بلاغ #${request.id}`} description="تفاصيل البلاغ وإجراءات سير العمل المدعومة." actions={<div className="flex flex-wrap gap-2"><Button asChild className="rounded-full" variant="outline"><Link to={`/maintenance/${request.id}/edit`}>تعديل</Link></Button><Button className="rounded-full" variant="outline" onClick={() => setAssignOpen(true)}>إسناد</Button><Button className="rounded-full" onClick={() => setTransitionOpen(true)}>تحديث الحالة</Button><Button className="rounded-full" variant="destructive" onClick={() => setDeleteOpen(true)}>حذف</Button></div>} />
