...
export function MaintenanceCreatePage() {
  ...
  return <motion.div {...pageMotion}><PageContainer><PageHeader eyebrow="الصيانة" title="بلاغ صيانة جديد" description="تسجيل بلاغ صيانة جديد وربطه بالعقار أو الوحدة عند الحاجة." />{failed ? <ErrorState title="تعذر تحميل بيانات النموذج" description="تحتاج الصفحة إلى العقارات والموردين." onRetry={() => { void propertiesQuery.refetch(); void vendorsQuery.refetch(); }} /> : <MaintenanceForm mode="create" properties={propertiesQuery.data ?? []} vendors={vendorsQuery.data ?? []} onSubmit={(payload) => submit(payload as MaintenanceCreate)} onCancelHref="/maintenance" />}</PageContainer></motion.div>;
}
