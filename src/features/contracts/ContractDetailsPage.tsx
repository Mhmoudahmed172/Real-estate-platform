...
export function ContractDetailsPage() {
  ...
  return <motion.div {...pageMotion}><PageContainer>
    <PageHeader eyebrow="العقود" title={`عقد #${contract.id}`} description="تفاصيل العقد وجدول الدفعات المرتبط به من backend." actions={<div className="flex flex-wrap gap-2"><Can permission="contracts.update"><Button asChild className="rounded-full" variant="outline"><Link to={`/contracts/${contract.id}/edit`}>تعديل</Link></Button><Button className="rounded-full" variant="outline" onClick={() => setRenewOpen(true)}>تجديد</Button><Button className="rounded-full" variant="outline" onClick={() => setCancelOpen(true)}>إلغاء</Button><Button className="rounded-full" variant="destructive" onClick={() => setTerminateOpen(true)}>إنهاء</Button></Can></div>} />
