...
export function ContractsPage() {
  ...
  const status = ((searchParams.get("status_filter") ?? searchParams.get("status")) as ContractStatus | null) ?? null;
  const expiringWithinDays = searchParams.get("expiring_within_days") ? Number(searchParams.get("expiring_within_days")) : null;
  ...
  const params = useMemo(() => ({ property_id: propertyId, owner_id: ownerId, tenant_id: tenantId, status_filter: status, expiring_within_days: expiringWithinDays && Number.isFinite(expiringWithinDays) ? expiringWithinDays : null, page: pagination.page, page_size: pagination.page_size }), [expiringWithinDays, ownerId, pagination.page, pagination.page_size, propertyId, status, tenantId]);
  ...
  const columns: Array<DataTableColumn<ContractOut>> = [
    { id: "contract", header: "العقد", cell: (row) => <div className="flex min-w-0 items-center gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary-soft text-primary"><ClipboardList aria-hidden="true" className="size-4" /></span><div className="min-w-0"><p className="font-semibold text-foreground">{tenantNames.get(row.tenant_id) ?? missingLabel("tenant")}</p><p className="text-meta">{propertyNames.get(row.property_id) ?? missingLabel("property")}</p></div></div> },
    { id: "tenant", header: "المستأجر", cell: (row) => tenantNames.get(row.tenant_id) ?? missingLabel("tenant") },
    { id: "owner", header: "المالك", cell: (row) => ownerNames.get(row.owner_id) ?? missingLabel("owner") },
  ...
  return <motion.div {...pageMotion}><PageContainer>
    <PageHeader eyebrow="إدارة العقود" title="العقود" description="متابعة العقود حسب العقار والمالك والمستأجر والحالة." actions={<Can permission="contracts.create"><Button asChild className="rounded-full shadow-sm"><Link to="/contracts/new"><Plus aria-hidden="true" className="size-4" /> إنشاء عقد</Link></Button></Can>} />
    ...
    {expiringWithinDays ? <ActiveFilterBanner description="العقود السارية التي ينتهي أجلها خلال المدة المحددة." label={`تنتهي خلال ${expiringWithinDays} يومًا`} /> : null}
    <section className="rounded-xl border border-border bg-card p-4 shadow-card"><div className="flex items-center justify-between gap-3"><div><h2 className="text-section text-foreground">عقود تنتهي خلال 30 يومًا</h2><p className="text-meta">العقود السارية التي يقترب موعد انتهائها.</p></div><span className="font-numeric text-kpi text-primary">{expiringQuery.data?.length ?? "—"}</span></div></section>
